...
Modify this document to analyze either Calories per Serving. Change the `ConfidenceBound` function to call `qt` with `n-1` degrees of freedom, instead of `qnorm` Edit the formula for confidence interval to state $t$ with $n-1$ degrees of freedom instead of $z$.
...
Use standard formula for standard error $\widehat{se} = \sigma / \sqrt{n}$ and confidence interval, assumeing a significance level $\alpha$ of 5%.

$$
CI(95\%) = \widehat{\mu} \pm t_{\alpha/2, n-1} \times \widehat{se}
$$.

*PMC R Function qt(p, df)*
```{r}
StandardError <- function(sigma, n) {
  sigma/sqrt(n)
}
ConfidenceBound <- function(sigma, n, alpha = 0.05) {
qt(1-alpha/2, n-1)*StandardError(sigma,n)
}
```
Create a variable for plotting and calculate upper and lower bounds using confidence intervals.

*PMC changed Calories per recipe to Calories to Serving*
...
PlotCookingTooMuch.dat$Lower <- PlotCookingTooMuch.dat$CaloriesPerServing - ConfidenceBound(CookingTooMuch.dat$CaloriesPerServingSD, n)
PlotCookingTooMuch.dat$Upper <- PlotCookingTooMuch.dat$CaloriesPerServing + ConfidenceBound(CookingTooMuch.dat$CaloriesPerServingSD, n)
...
Check our calculations.  Wansink reports 95% confidence intervals for the 1936 and 2006 means as [1638.7, 2608.9] and [2360.7, 3743.1], respectively. We should be equal, to within rounding error.
